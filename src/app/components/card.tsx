...
export function Card({filme} : CardProps){
    ...
    return(
        <div className="flex flex-col items-center w-52 gap-2 relative">
            {favorito ? <Heart onClick={toggleFavorito} className="absolute text-red-600 right-2 top-2 cursor-pointer"/> : <Heart onClick={toggleFavorito} className="absolute right-2 top-2 cursor-pointer"/>    
                }
